...
- `agente-home.html` — dashboard: crear cliente, buscar propiedades, ley, resumir link. Link a "Leads del día" (todos lo ven ahora, filtrado por asignación). Para Admin: la lista completa "Mis clientes" (cards de todos los clientes de todos los agentes) está oculta — en su lugar hay un widget "Clientes por agente" debajo de "Resumen de clientes" con un botón por cada agente (Damara/Moisés/Guillermo); al picarlo abre un popup con una tabla simple (nombre + estado: Cliente potencial / Listo para asesor / Portal creado - etapa), combinando leads de Chatwoot (`getLeadsPotenciales`) con clientes de Firestore (`listClients`) filtrados por `agent_uid`/`assignee_uid`. Para agentes no-admin, "Mis clientes" se sigue mostrando normal (su propia lista de cards).
  - **Buscar propiedades:** cada resultado trae burbuja de comisión (`.prop-badge`, cualquier dato de EasyBroker que le convenga al agente vender pronto). Botones "Llamar"/"WhatsApp" para confirmar disponibilidad con el agente dueño de la ficha — SOLO aparecen si es un agente EXTERNO (el backend ya filtra los 3 de Tres65, no hace falta filtrar aquí). Sección aparte **"O agrega una propiedad por link"**: pega cualquier link de EasyBroker (incluyendo fichas de la "bolsa" de otra organización) → botón "Procesar propiedad" muestra preview (título como hipervínculo, foto, comisión o input manual si no se pudo sacar) → nota para el agente → "Agregar a cliente". En el shortlist de Admin, cada propiedad agregada a un cliente tiene "Link a guardar"/"Nota para el agente" editables.
  - **Botón "Mensajes EasyBroker"** (top-nav, badge con conteo): abre modal con las inquietudes de `/contact_requests` que el cron `check_easybroker_mensajes` (backend) aún no marcó atendidas — nombre, ficha, mensaje, Llamar/WhatsApp con el teléfono del interesado, "Marcar como atendido". Si el mensaje no generó lead automático (ficha de la oficina sin agente individual, o sin teléfono — ver `agente-tres65/CLAUDE.md`), Admin ve un botón extra **"Crear lead"** que abre el mismo formulario del lead manual precargado, ligado por `mensaje_id` para no duplicar.
- `cliente-detalle.html` — detalle de un cliente: datos (incluye Correo), tipo de comprador + estrategia de cierre + burbuja de prioridad (alta/media/baja), propiedad de interés, propiedades agregadas (buscar/agregar por link/quitar), avance del progreso (checklist con palomitas + burbujas + reporte retráctil), y "Tareas" (antes "Notas internas" — ahora cada una tiene checkbox, al marcarla se tacha; el campo en Firestore sigue llamándose `agent_notes` con un `done: bool` nuevo, sin migración de datos viejos). Debajo de las propiedades agregadas hay dos botones (`buildPropertiesListCard`): **"Análisis pre-tour"** (el análisis comparativo de siempre: generar/corregir/compartir) y **"Ruta del tour"** (2026-08-25, fase 1 — sin API de pago) que arma un link de Google Maps multi-parada (`https://www.google.com/maps/dir/?api=1&origin=...&destination=...&waypoints=...`) usando `latitude`/`longitude` que YA vienen en `entry.data` de cada propiedad (las pone `_report_property_summary` en el backend, se resuelven de EasyBroker sin geocodificar nada aparte) — pide el punto de partida con un `prompt()` y abre Maps en pestaña nueva. Fase 2 pendiente (ver `project_tres65_pendientes` en memoria): conectar la API de Directions de Google (esa sí con costo/key) para sacar kilometraje y tiempo real, ligado al pendiente de "costos operativos del recorrido" de la reunión del 20 de agosto.
- `leads.html` — tablero de leads desde Chatwoot (cliente-potencial / listo-para-asesor), Round Robin (solo Admin), convertir/crear portal, registrar intentos de contacto (1er/2do/... contacto: contestó o no + resultado, se acumulan, se ven todos en la tarjeta). Botón "Borrar lead" (rojo suave, pide razón) le pone el label `lead-perdido` en Chatwoot — no borra la conversación, solo lo saca del tablero activo. Toggle "Ver leads perdidos"/`?view=perdidos` en la misma página muestra esos leads con su razón/quién/cuándo.
  - **Directorio** (modal en `agente-home.html`, botón "Directorio" en el top-nav): fila cerrada Nombre/Celular/Email/Tipo de contacto/Asignado a + flecha para expandir el detalle (estado, datos del tipo, acciones). "Sin asignar" es una opción real y seleccionable en "Asignar a" (antes un placeholder deshabilitado sin opción para dejarlo vacío — el bug real). Tipo de contacto (Cliente/Asesor externo/Personal interno TRES65/Proveedor de servicios) + estado de Cliente (activo/cierre ganado con notas/cierre perdido) se editan con el fieldset compartido `tipoFieldsetHTML`/`wireTipoFieldset`/`setTipoFieldsetValues`/`readTipoFieldsetValues` (mismo HTML y funciones en el modal "Agregar/Editar manual" y en el detalle de cualquier fila), guardado vía `setDirectorioTipo(row_key, payload)` → `POST /portal/directorio/tipo` (backend en `agente-tres65/CLAUDE.md`). "Reactivar como lead" (solo Cliente/Asesor externo) vive en el detalle → `reactivarComoLead(row_key, agent_uid?)`. Cada contacto tiene "Compartir por WhatsApp"/"Copiar contacto" (`buildContactShareText`, solo campos existentes, sin backend). El modal "Editar" en lote (checkboxes, exportar CSV, borrar con contraseña) es una capa aparte, sin relación con lo anterior.
    - **Permisos (backend manda, frontend solo refleja):** el botón "Editar" (lápiz) de una fila manual sale si `e.puede_editar_tipo` (Admin/Moises en cualquiera, un agente normal solo en las suyas — ya NO es "solo Admin"). "Importar CSV" se oculta por completo (`display:none`) si la sesión no es Admin/Moises (el backend ya lo rechazaba con 403; esto es solo UX). "+ Agregar manual" sigue visible a cualquier agente. El selector "Asignar a" del modal solo aparece para Admin/Moises — un agente normal editando su propio contacto nunca ve ni manda ese campo (el backend lo ignora de todos modos).
...
Instalado (ID `516732134278228`) en `index.html`, `home.html`, `agente.html`, `resumen.html` — NO en las páginas de agentes/admin (`agente-home.html`, `agente-login.html`, `cliente-detalle.html`, `leads.html`), esas no son de cara al cliente. `PageView` automático, `Lead` al completar Discovery, `DiscoveryStep` custom (para medir abandono por pregunta). Ver `agente-tres65/CLAUDE.md` para la parte de Conversions API (server-side, venta cerrada).
